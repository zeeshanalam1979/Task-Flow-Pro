    ...
    for proj in projects:
        ...
        for t in pt:
            ...
            <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid #f0f1f4;font-size:14px">
                <span style="font-size:16px">{STATUS_ICONS.get(t['status'],'⬜')}</span>
                <span style="flex:1;font-weight:500;color:#172b4d">{t['title']}</span>
                {'<span style="font-size:12px;color:#5e6c84">☑ '+str(sub_done)+'/'+str(len(subs))+'</span>' if subs else ''}
                <span style="font-family:monospace;font-size:12px;color:#5e6c84;background:#f0f1f4;padding:2px 8px;border-radius:4px">{fmt_time(t.get('elapsed',0) or 0)}</span>
                {badge_html(t['status'])}
            </div>""", unsafe_allow_html=True)
...
if st.session_state.board_view == "kanban":
    ...
    for col_idx, (status_key, status_label, status_color) in enumerate(KANBAN_COLS):
        ...
        with cols[col_idx]:
            ...
            for t in col_tasks:
                ...
                # Card
                st.markdown(f"""
                <div class="k-card" style="border-left:3px solid {status_color}">
                    <div class="k-card-title">{t['title']}</div>
                    <div class="k-card-meta">
                        <span class="pri-dot" style="background:{PRI_COLORS.get(t['priority'],'#5e6c84')}"></span>
                        <span style="font-size:12px;color:#5e6c84">{t['priority'].capitalize()}</span>
                        {'<span style="font-size:12px;color:#5e6c84">· 📅 '+t["due"]+'</span>' if t.get("due") else ''}
                        {'<span style="font-size:12px;color:#5e6c84">· ☑ '+str(sub_done)+'/'+str(len(subs))+'</span>' if subs else ''}
                        {'<span style="font-size:12px;color:#5e6c84">· 💬 '+str(len(coms))+'</span>' if coms else ''}
                    </div>
                    <div class="k-card-footer">
                        <span style="font-size:12px;color:#5e6c84;background:#f0f1f4;padding:2px 8px;border-radius:4px;font-family:monospace">⏱ {fmt_time(elapsed)}{'  ⬤' if is_run else ''}</span>
                        <span style="font-size:12px;color:#5e6c84">📁 {proj['name'] if proj else '—'}</span>
                    </div>
                </div>""", unsafe_allow_html=True)
    ...
    for t in filtered_sorted:
        ...
        sub_bar  = f'<div style="background:#dfe1e6;border-radius:3px;height:4px;width:60px;display:inline-block;vertical-align:middle"><div style="background:#0052cc;height:100%;border-radius:3px;width:{round(sub_done/len(subs)*100) if subs else 0}%"></div></div> {sub_done}/{len(subs)}' if subs else "—"
        ...
        st.markdown(f"""
        <tr>
          <td style="text-align:center;font-size:16px">{st_icon}</td>
          <td>
            <div class="task-title-cell" style="{title_style}">{t['title']}</div>
            {'<div class="task-desc-cell">'+t["desc"][:80]+('...' if len(t.get("desc",""))>80 else '')+'</div>' if t.get("desc") else ''}
          </td>
          <td>{badge_html(t['status'])}</td>
          <td><span style="font-size:14px">{pri_dot}{t['priority'].capitalize()}</span></td>
          <td><span style="font-size:13px;color:{pc};font-weight:600">● {proj['name'] if proj else '—'}</span></td>
          <td><span style="font-size:14px">👤 {t.get('assignee','—')}</span></td>
          <td><span style="font-size:14px;color:#172b4d">{'📅 '+t['due'] if t.get('due') else '—'}</span></td>
          <td><span class="timer-pill {'running' if is_run else ''}">⏱ {fmt_time(elapsed)}{run_ind}</span></td>
          <td><span style="font-size:13px">{sub_bar}</span></td>
          <td></td>
