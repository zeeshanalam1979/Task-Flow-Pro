...
if st.session_state.board_view == "kanban":
    # ── KANBAN BOARD — rendered as one self-contained HTML block ──────────────
    import streamlit.components.v1 as components

    def build_kanban_html(filtered_tasks, projects_list, active_timer_id):
        """Build the entire kanban board as a single valid HTML string."""

        def _badge(status):
            colors = {"todo":("#5e6c84","#f0f1f4"),"in-progress":("#0052cc","#e3f2fd"),
                      "review":("#5e35b1","#ede7f6"),"blocked":("#c62828","#ffebee"),"done":("#2e7d32","#e8f5e9")}
            c, bg = colors.get(status, ("#5e6c84","#f0f1f4"))
            label = {"todo":"TO DO","in-progress":"IN PROGRESS","review":"IN REVIEW",
                     "blocked":"BLOCKED","done":"DONE"}.get(status, status.upper())
            return f'<span style="background:{bg};color:{c};padding:2px 8px;border-radius:4px;font-size:11px;font-weight:700">{label}</span>'

        def _proj_name(pid):
            p = next((x for x in projects_list if x["id"]==pid), None)
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; }
        ...
        .board { display: flex; gap: 10px; overflow-x: auto; padding: 4px 2px 12px; align-items: flex-start; }
        .col { min-width: 220px; flex: 1; background: #f4f5f7; border-radius: 8px; padding: 8px; }
        .col-header { display: flex; align-items: center; justify-content: space-between;
                      padding: 6px 8px 10px; }
        .col-label { font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: .06em; }
        .col-count { color: #fff; border-radius: 10px; padding: 1px 8px; font-size: 11px; font-weight: 700; }
        .card { background: #fff; border-radius: 8px; padding: 12px; margin-bottom: 8px;
                border: 1px solid #dfe1e6; border-left-width: 3px; }
        .card-title { font-size: 14px; font-weight: 600; color: #172b4d; margin-bottom: 8px; line-height: 1.4; }
        .card-meta { display: flex; flex-wrap: wrap; align-items: center; gap: 5px; margin-bottom: 8px; }
        .pri-dot { width: 9px; height: 9px; border-radius: 50%; display: inline-block; }
        .meta-txt { font-size: 12px; color: #5e6c84; }
        .card-footer { display: flex; justify-content: space-between; align-items: center;
                       padding-top: 8px; border-top: 1px solid #f0f1f4; }
        .timer-chip { font-family: monospace; font-size: 12px; background: #f0f1f4;
                      color: #5e6c84; padding: 2px 8px; border-radius: 4px; }
        .timer-chip.running { background: #e3f2fd; color: #0052cc; }
        .proj-tag { font-size: 12px; color: #5e6c84; }
        .empty { font-size: 12px; color: #adb5bd; text-align: center; padding: 20px 0; }
        </style>
        """

        pri_colors = {"high":"#ef4444","medium":"#f59e0b","low":"#22c55e"}

        html_parts = [css, '<div class="board">']

        for (status_key, status_label, status_color) in cols_cfg:
            col_tasks = [t for t in filtered_tasks if t["status"] == status_key]
            count = len(col_tasks)

            html_parts.append(
                f'<div class="col">'
                f'<div class="col-header">'
                f'<span class="col-label" style="color:{status_color}">{status_label}</span>'
                f'<span class="col-count" style="background:{status_color}">{count}</span>'
                f'</div>'
            )

            if not col_tasks:
                html_parts.append('<div class="empty">No tasks</div>')
            else:
                for t in col_tasks:
                    subs      = jl(t.get("subtasks"))
                    coms      = jl(t.get("comments"))
                    sub_done  = sum(1 for s in subs if s["done"])
                    elapsed   = int(t.get("elapsed", 0) or 0)
                    is_run    = t["id"] == active_timer_id
                    pri_col   = pri_colors.get(t["priority"], "#5e6c84")
                    pname     = _proj_name(t["project_id"])
                    tmr_str   = fmt_time(elapsed)
                    run_cls   = " running" if is_run else ""
                    run_dot   = " ●" if is_run else ""

                    # Meta line pieces
                    meta = (
                        f'<span class="pri-dot" style="background:{pri_col}"></span>'
                        f'<span class="meta-txt">{t["priority"].capitalize()}</span>'
                    )
                    if t.get("due"):
                        meta += f'<span class="meta-txt">· 📅 {t["due"]}</span>'
                    if subs:
                        meta += f'<span class="meta-txt">· ☑ {sub_done}/{len(subs)}</span>'
                    if coms:
                        meta += f'<span class="meta-txt">· 💬 {len(coms)}</span>'

                    html_parts.append(
                        f'<div class="card" style="border-left-color:{status_color}">'
                        f'<div class="card-title">{t["title"]}</div>'
                        f'<div class="card-meta">{meta}</div>'
                        f'<div class="card-footer">'
                        f'<span class="timer-chip{run_cls}">⏱ {tmr_str}{run_dot}</span>'
                        f'<span class="proj-tag">📁 {pname}</span>'
                        f'</div>'
                        f'</div>'
                    )

            html_parts.append('</div>')  # close .col

        html_parts.append('</div>')  # close .board
        return "".join(html_parts)

    # Render kanban as one clean HTML component
    board_html = build_kanban_html(filtered, projects, st.session_state.active_timer)
    num_cols   = len(KANBAN_COLS)
    card_count = len(filtered)
    board_h    = max(300, 160 + (card_count // num_cols + 1) * 130)
    components.html(board_html, height=board_h, scrolling=True)

    # ── Per-task action buttons (native Streamlit, below board) ───────────────
    st.markdown("---")
    st.markdown("**Task Actions** — select a task to manage:")
    ...
    for t in filtered:
        ...
        proj    = get_proj(t["project_id"])
        sc      = STATUS_COLORS.get(t["status"], "#5e6c84")

        # Task row with colored left border indicator
        st.markdown(
            f'<div style="border-left:3px solid {sc};padding:6px 10px;background:#fff;'
            f'border-radius:6px;margin-bottom:4px;border:1px solid #dfe1e6">'
            f'<span style="font-size:14px;font-weight:600;color:#172b4d">{t["title"]}</span>'
            f'&nbsp;&nbsp;<span style="font-size:12px;color:#5e6c84">{proj["name"] if proj else ""}'
            f' · {STATUS_LABELS.get(t["status"],"")}</span>'
            f'</div>',
            unsafe_allow_html=True
        )

        ba, bb, bc, bd, be = st.columns([1, 1, 1, 1, 6])
        with ba:
            lbl = "⏹" if is_run else "▶"
            if st.button(lbl, key=f"tmr_{t['id']}", use_container_width=True,
                         help="Stop timer" if is_run else "Start timer"):
                if is_run:
                    if st.session_state.timer_start:
                        added = int((datetime.now()-st.session_state.timer_start).total_seconds())
                        db_update_task(t["id"], {"elapsed": elapsed + added})
        ...
        with bb:
            ...
            if new_st != t["status"]:
                db_update_task(t["id"], {"status": new_st}); st.rerun()
        with bc:
            if st.button("💬", key=f"exp_{t['id']}", use_container_width=True, help="Details/comments"):
        ...
        with bd:
            if st.button("✏️", key=f"ed_{t['id']}", use_container_width=True, help="Edit"):
                ...
                st.session_state.show_add_task = False; st.rerun()
